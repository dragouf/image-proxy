...
function retrieve(remote, req, res, next) {
    ...
    var request = agent.get(options, function (response) {
        ...
        if ((response.statusCode === 301 || response.statusCode === 302) && response.headers['location']) {
          ...
          return retrieve(url.format(redirect), req, res, next);
        }
        ...
        // if octet/stream try to get type from file name
        if(mimeType === 'binary/octet-stream') {
          mimeType = mime.getType(path.extname(options.pathname || '.jpg'))
        }
...
module.exports = function () {
  app.get('/:url?', function (req, res, next) {
    ...
    retrieve(req.params.url, req, res, next);
  });
